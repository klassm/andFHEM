...
object AvailableTargetStatesDialogUtil {
    ...
    fun showSwitchOptionsMenu(context: Context, device: FhemDevice, callback: OnTargetStateSelectedCallback) {
        ...
        val eventMapOptions = device.availableTargetStatesEventMapTexts
        ...
        val adapter = SetListArrayAdapter(context, eventMapOptions, setOptions, device)
        contextMenu.setAdapter(adapter, clickListener)
        contextMenu.show()
    }

    fun <D : FhemDevice?> showSwitchOptionsMenuFor(context: Context?, device: D, callback: OnTargetStateSelectedCallback) {
        val entry = device!!.setList["state", true]
        handleSelectedOption(context, device, entry, callback)
    }

    fun <D : FhemDevice?> handleSelectedOption(context: Context?, device: D, option: SetListEntry, callback: OnTargetStateSelectedCallback): Boolean {
