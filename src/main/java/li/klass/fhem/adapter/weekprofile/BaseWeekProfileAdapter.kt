...
abstract class BaseWeekProfileAdapter<INTERVAL : BaseHeatingInterval<INTERVAL>> internal constructor(protected val context: Context) : NestedListViewAdapter<DayProfile<INTERVAL, HeatingIntervalConfiguration<INTERVAL>>, INTERVAL>(context) {
    ...
    override fun getChildrenCountForParent(parent: DayProfile<INTERVAL, HeatingIntervalConfiguration<INTERVAL>>): Int {
    ...
    override fun getChildForParentAndChildPosition(parent: DayProfile<INTERVAL, HeatingIntervalConfiguration<INTERVAL>>, childPosition: Int): INTERVAL? {
    ...
    private fun getParentTextFor(profile: DayProfile<INTERVAL, HeatingIntervalConfiguration<INTERVAL>>?): String {
        when {
            profile != null -> return resources.getText(profile.day.stringId).toString()
            else -> return "??"
        }
    }
    ...
    private fun showCopyContextMenuFor(target: DayProfile<INTERVAL, *>) {
        ...
        val parents = parents
        val selectOptions = FluentIterable.from(parents)
                .transform { input -> getParentTextFor(input) }.toList()
        ...
        contextMenu.setAdapter(ArrayAdapter(context, android.R.layout.simple_list_item_1, selectOptions), clickListener)
        contextMenu.show()
    }
    ...
    override fun getParents(): List<DayProfile<INTERVAL, HeatingIntervalConfiguration<INTERVAL>>> {
        ...
        val sortedDayProfiles = weekProfile!!.getSortedDayProfiles()
    ...
    fun timeToTimeString(hourOfDay: Int, minuteOfDay: Int): String {
        ...
        val intervalMinutesMustBeDivisibleBy = weekProfile!!.getConfiguration().getIntervalMinutesMustBeDivisibleBy()
        var minutes = (minuteOfDay + intervalMinutesMustBeDivisibleBy - 1) / intervalMinutesMustBeDivisibleBy * intervalMinutesMustBeDivisibleBy
        if (minutes == 60) minutes = 0

        if (minutes == 0 && minuteOfDay != 0) myHour += 1

        return String.format(Locale.getDefault(), "%02d", myHour) + ":" + String.format(Locale.getDefault(), "%02d", minutes)
    }
